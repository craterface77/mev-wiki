    ...
    describe "CLI" $ do
      ...
      it "print-state-merge-debug" $ do
        Just c <- runApp $ solcRuntime (T.pack "C") (T.pack [i|
          ...
          |])
        let hexStr = Types.bsToHex c
        (exitCode, stdout, stderr) <- readProcessWithExitCode "cabal" ["run", "exe:hevm", "--", "symbolic", "--debug", "--solver", "bitwuzla", "--code", hexStr] ""
        (T.count "Merged forward jump at addr:" (T.pack stderr)) `shouldBe` 4
        stdout `shouldContain` "Discovered the following"
        exitCode `shouldBe` (ExitFailure 1)

      it "print-state-merge-debug-lines" $ do
        (exitCode, stdout, stderr) <- runForge "test/contracts/fail/merge-test.sol" ["--debug"]
        (T.count "Merged forward jump in file" (T.pack stderr)) `shouldBe` 4
        stdout `shouldContain` "Counterexample:"
        exitCode `shouldBe` (ExitFailure 1)
